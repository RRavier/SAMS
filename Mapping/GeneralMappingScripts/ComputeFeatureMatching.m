    ...
    if i ~= frechMean
        ...
        for j = 1:length(map_12)
            ...
            meshList{frechMean}.V(:,featureList{frechMean}(map_12(j)))');
                weightAdj1 = graph(pdist2(meshList{i}.V',meshList{i}.V').*meshList{i}.A);
                weightAdj2 = graph(pdist2(meshList{frechMean}.V',meshList{frechMean}.V')...
                    .*meshList{frechMean}.A);
                [~,dist12,~] = shortestpath(weightAdj2,lmk12,...
                    featureList{frechMean}(map_12(j)));
                [~,dist21,~] = shortestpath(weightAdj1,lmk21,...
                   featureList{i}(j));
